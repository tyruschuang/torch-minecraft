import { Typography } from "@mui/material";
import axios from "axios";

const specialWordStyle = {
  color: "#ffc247",
  fontFamily: '"Fira Mono", monospace',
};

export const sections = [
  {
    title: "Getting started",
    description: (
      <Typography fontSize={18} marginTop={2}>
        The Torch API is public, read-only, and returns{" "}
        <span style={specialWordStyle}>JSON</span>. Every route uses the{" "}
        <span style={specialWordStyle}>GET</span> method, so you can call it
        from a browser, command-line client, or application without an API key.
        <br />
        <br />
        Start with{" "}
        <span style={specialWordStyle}>/status/auto/&lt;address&gt;</span>{" "}
        unless you already know the edition. URL-encode server addresses before
        placing them in a route.
      </Typography>
    ),
  },
  {
    title: "Caching",
    description: (
      <Typography fontSize={18} marginTop={2}>
        Java and Bedrock status responses are cached for 30 seconds. SRV records
        and server icons are cached for 30 minutes. Repeated requests made
        during that window may return the same response.
        <br />
        <br />
        Use <span style={specialWordStyle}>obtained_at</span> and{" "}
        <span style={specialWordStyle}>expires_at</span> to determine when
        cached data was collected and when Torch will refresh it.
      </Typography>
    ),
  },
  {
    title: "Errors and unavailable servers",
    description: (
      <Typography fontSize={18} marginTop={2}>
        A valid status request returns <span style={specialWordStyle}>200</span>
        , even when the target server does not respond. In that case, the
        response includes <span style={specialWordStyle}>offline: true</span>.
        Use the diagnostics route for edition-specific timeout, DNS, refused
        connection, and protocol details.
        <br />
        <br />
        Invalid addresses or ports return{" "}
        <span style={specialWordStyle}>400</span>.
      </Typography>
    ),
  },
];

export const endpoints = [
  {
    title: "Automatic Edition Detection",
    route: "status/auto/<ip>",
    description: (
      <Typography
        fontSize={16}
        paddingLeft={2}
        marginBottom={2}
        paddingRight={2}
      >
        Probes Java and Bedrock concurrently. If Java responds, Torch returns
        the Java status; otherwise, it returns Bedrock. The response also
        includes diagnostics for both probes.
      </Typography>
    ),
    response: {
      edition: "java",
      status: {
        host: "hypixel.net",
        port: 25565,
        version: {
          name: "Requires MC 1.8 / 1.21",
          protocol: 772,
        },
        players: {
          online: 32000,
          max: 200000,
        },
      },
      diagnostics: {
        requested_host: "hypixel.net",
        dns_addresses: ["104.17.71.15"],
        java: {
          edition: "java",
          reachable: true,
          host: "hypixel.net",
          port: 25565,
          version: "Requires MC 1.8 / 1.21",
          protocol: 772,
          latency: 24,
        },
        bedrock: {
          edition: "bedrock",
          reachable: false,
          host: "hypixel.net",
          port: 19132,
          error_code: "timeout",
          error_message: "The server did not respond before the timeout.",
        },
      },
    },
  },
  {
    title: "Connection Diagnostics",
    route: "diagnostics/<ip>",
    description: (
      <Typography
        fontSize={16}
        paddingLeft={2}
        marginBottom={2}
        paddingRight={2}
      >
        Returns DNS addresses, resolved SRV targets, port reachability, protocol
        versions, latency, and actionable Java and Bedrock connection errors.
      </Typography>
    ),
    response: {
      requested_host: "minehut.gg",
      dns_addresses: ["104.18.40.155", "172.64.147.101"],
      java: {
        edition: "java",
        reachable: true,
        host: "minehut.gg",
        port: 25565,
        resolved_host: "java-us.minehut.com",
        resolved_port: 25565,
        version: "Velocity 1.7.2-26.2",
        protocol: 772,
        latency: 19,
      },
      bedrock: {
        edition: "bedrock",
        reachable: false,
        host: "minehut.gg",
        port: 19132,
        error_code: "timeout",
        error_message: "The server did not respond before the timeout.",
      },
    },
  },
  {
    title: "Java Server Status",
    route: "status/java/<ip>",
    description: (
      <Typography
        fontSize={16}
        paddingLeft={2}
        marginBottom={2}
        paddingRight={2}
      >
        Returns one Java Edition server's status. Replace{" "}
        <span style={specialWordStyle}>{"<ip>"}</span> with a hostname or IP
        address, optionally followed by a port—for example,{" "}
        <span style={specialWordStyle}>hypixel.net</span> or{" "}
        <span style={specialWordStyle}>example.net:25570</span>. Without a port,
        Torch uses <span style={specialWordStyle}>25565</span>.
      </Typography>
    ),
    response: {
      host: "hypixel.net",
      port: 25565,
      version: {
        name: {
          raw: "Requires MC 1.8 / 1.21",
          clean: "Requires MC 1.8 / 1.21",
          html: '<span><span style="">Requires&nbsp;MC&nbsp;1.8&nbsp;/&nbsp;1.21</span></span>',
          json: '{"1":{"text":"Requires MC 1.8 / 1.21","styles":[]}}',
          segments: [
            {
              text: "Requires MC 1.8 / 1.21",
              styles: [],
            },
          ],
        },
        protocol: 772,
      },
      players: {
        max: 200000,
        online: 42102,
        sample: [],
      },
      description: {
        raw: "§f                 §aHypixel Network §c[1.8/26.2]\n§f   §6§lSB 0.27 TORRHUS & SAFARI §7| §e§lSUMMER EVENT",
        clean:
          "                 Hypixel Network [1.8/26.2]\n   SB 0.27 TORRHUS & SAFARI | SUMMER EVENT",
        html: '<span><span style="color: #ffffff; ">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span><span style="color: #55ff55; ">Hypixel&nbsp;Network&nbsp;</span><span style="color: #ff5555; ">[1.8/26.2]<br /></span><span style="color: #ffffff; ">&nbsp;&nbsp;&nbsp;</span><span style="color: #ffaa00; font-weight: bold; ">SB&nbsp;0.27&nbsp;TORRHUS&nbsp;&amp;&nbsp;SAFARI&nbsp;</span><span style="color: #aaaaaa; ">|&nbsp;</span><span style="color: #ffff55; font-weight: bold; ">SUMMER&nbsp;EVENT</span></span>',
        json: '{"1":{"text":"                 ","styles":["color=#ffffff"]},"2":{"text":"Hypixel Network ","styles":["color=#55ff55"]},"3":{"text":"[1.8/26.2]\\n","styles":["color=#ff5555"]},"4":{"text":"   ","styles":["color=#ffffff"]},"5":{"text":"SB 0.27 TORRHUS & SAFARI ","styles":["color=#ffaa00","bold"]},"6":{"text":"| ","styles":["color=#aaaaaa"]},"7":{"text":"SUMMER EVENT","styles":["color=#ffff55","bold"]}}',
        segments: [
          {
            text: "                 ",
            styles: ["color=#ffffff"],
          },
          {
            text: "Hypixel Network ",
            styles: ["color=#55ff55"],
          },
          {
            text: "[1.8/26.2]\n",
            styles: ["color=#ff5555"],
          },
          { text: "   ", styles: ["color=#ffffff"] },
          {
            text: "SB 0.27 TORRHUS & SAFARI ",
            styles: ["color=#ffaa00", "bold"],
          },
          { text: "| ", styles: ["color=#aaaaaa"] },
          {
            text: "SUMMER EVENT",
            styles: ["color=#ffff55", "bold"],
          },
        ],
      },
      icon: "data:image/png;base64,",
      mod_info: null,
      used_srv: {
        host: "mc.hypixel.net.",
        port: 25565,
      },
      latency: 66,
      obtained_at: "2023-05-19T18:14:51.3294472-07:00",
      expires_at: "2023-05-19T18:19:51.3294472-07:00",
    },
  },
  {
    title: "Bedrock Server Status",
    route: "status/bedrock/<ip>",
    description: (
      <Typography
        fontSize={16}
        marginBottom={2}
        paddingLeft={2}
        paddingRight={2}
      >
        Returns one Bedrock Edition server's status. Replace{" "}
        <span style={specialWordStyle}>{"<ip>"}</span> with a hostname or IP
        address, optionally followed by a port—for example,{" "}
        <span style={specialWordStyle}>play.craftersmc.net</span> or{" "}
        <span style={specialWordStyle}>example.net:19133</span>. Without a port,
        Torch uses <span style={specialWordStyle}>19132</span>.
      </Typography>
    ),
    response: {
      server_guid: "17815442013524260113",
      version: {
        name: {
          raw: "1.26.44",
          clean: "1.26.44",
          html: '<span><span style="">1.26.44</span></span>',
          json: '{"1":{"text":"1.26.44","styles":[]}}',
          segments: [{ text: "1.26.44", styles: [] }],
        },
        protocol: 2168,
      },
      edition: "MCPE",
      motd: {
        raw: "CraftersMC ✧ SkyBlock: End § Beyond!\n§b§lPlay Now!§r",
        clean: "CraftersMC ✧ SkyBlock: End § Beyond!\nPlay Now!",
        html: '<span><span style="">CraftersMC&nbsp;✧&nbsp;SkyBlock:&nbsp;End&nbsp;§&nbsp;Beyond!<br /></span><span style="color: #55ffff; font-weight: bold; ">Play&nbsp;Now!</span></span>',
        json: '{"1":{"text":"CraftersMC ✧ SkyBlock: End § Beyond!\\n","styles":[]},"2":{"text":"Play Now!","styles":["color=#55ffff","bold"]}}',
        segments: [
          {
            text: "CraftersMC ✧ SkyBlock: End § Beyond!\n",
            styles: [],
          },
          {
            text: "Play Now!",
            styles: ["color=#55ffff", "bold"],
          },
        ],
      },
      players: {
        max: 1000,
        online: 95,
        sample: null,
      },
      server_id: "17815442013524260113",
      gamemode: "Survival",
      gamemode_id: 1,
      port: 19132,
      port_ipv4: null,
      port_ipv6: null,
      host: "play.craftersmc.net",
      obtained_at: "2023-05-19T20:02:09.0892782-07:00",
      expires_at: "2023-05-19T20:07:09.0892782-07:00",
      latency: 99,
    },
  },
  {
    title: "Server Icon",
    route: "icon/<ip>",
    description: (
      <Typography
        fontSize={16}
        marginBottom={2}
        paddingLeft={2}
        paddingRight={2}
      >
        Returns a Java server's icon as a Base64 data URI. If the server doesn't
        advertise an icon or cannot be reached, Torch returns its default
        Minecraft server icon. Replace{" "}
        <span style={specialWordStyle}>{"<ip>"}</span> with the server address;
        the default port is <span style={specialWordStyle}>25565</span>.
      </Typography>
    ),
    response: {
      host: "minehut.com",
      port: 25565,
      data: "data:image/png;base64,",
      obtained_at: "2023-05-25T01:25:02.718884931Z",
      expires_at: "2023-05-25T01:55:02.718885051Z",
    },
  },
  {
    title: "SRV Record",
    route: "srv/<host>",
    description: (
      <Typography
        fontSize={16}
        marginBottom={2}
        paddingLeft={2}
        paddingRight={2}
      >
        Resolves the Minecraft Java SRV record for a hostname. If no record
        exists, Torch returns the original hostname with port{" "}
        <span style={specialWordStyle}>25565</span>. For example, try{" "}
        <span style={specialWordStyle}>minehut.gg</span>.
      </Typography>
    ),
    response: {
      target: "java.cox.minehut.com.",
      port: 25565,
      obtained_at: "2023-05-25T01:25:48.388355033Z",
      expires_at: "2023-05-25T01:55:48.388355153Z",
    },
  },
];

export const apiBaseUrl = (
  process.env.REACT_APP_API_URL || "http://localhost:8000"
).replace(/\/+$/, "");

const api = axios.create({
  baseURL: apiBaseUrl,
  timeout: 20000,
});

export default api;
