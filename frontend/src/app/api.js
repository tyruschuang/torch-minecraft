...
export const sections = [
	{
		title: "Standards",
		description: (
			<Typography fontSize={18} marginTop={2}>
				At Torch, we believe in simplicity and compatibility. That's why
				our API follows the <span style={specialWordStyle}>REST</span>{" "}
				standard, ensuring compatibility with a wide range of
				applications and systems. In simpler terms, you'll be making
				HTTP requests to the Torch service. All endpoints for the API
				return a response body in{" "}
				<span style={specialWordStyle}>JSON</span> format, which is easy
				to parse and work with. Additionally, the Torch API utilizes
				only endpoints using the{" "}
				<span style={specialWordStyle}>GET</span>
				method, making it easy to retrieve data quickly and efficiently.
				<br />
				<br />
				By following shared conventions and standards, we ensure that
				our API is easy to use and integrate into your existing
				projects. Clients built around our API can take advantage of its
				features, such as efficiently caching responses. This not only
				makes your application faster, but also reduces server load,
				making it more efficient and cost-effective.
				<br />
				<br />
				To get started with the Torch API, simply make an{" "}
				<span style={specialWordStyle}>HTTP</span> request to the
				desired endpoint, and you'll receive a response in{" "}
				<span style={specialWordStyle}>JSON</span> format. Our API
				documentation provides clear and concise information on all
				available endpoints, making it easy to integrate our API into
	...
	},
	{
		...
		description: (
			<Typography fontSize={18} marginTop={2}>
				Caching is an essential part of the Torch API, designed to
				reduce server load and provide an optimal user experience. Our
				API implements short cache times, which means that when you make
				a request to an endpoint and make the same request again before
				the cache expires, you will receive the same data in response.
				This significantly reduces the amount of time and resources
				required to retrieve data, ensuring that our API is fast and
				efficient.
				<br />
				<br />
				When you receive a response from our API, you will notice that
				it includes timestamp information indicating when the cache was
				obtained and when it expires. This information is essential for
				ensuring that you are always working with the most up-to-date
				data.
				<br />
				<br />
				At Torch, we understand that clear and concise communication is
				essential for providing the best user experience. That's why
				we've designed our caching system to be easy to understand and
				use. By implementing short cache times and providing timestamp
				information with each response, we ensure that our API is both
				fast and reliable.
			</Typography>
		),
	},
...
export const endpoints = [
	{
		...
		description: (
			...
			>
				...
				<span style={specialWordStyle}>{"<ip>"}</span> should be
				replaced with the server's hostname or IP address. A valid IP
				address can be <span style={specialWordStyle}>hypixel.net</span>{" "}
				or <span style={specialWordStyle}>hypixel.net:25565</span>. If
				you do not provide a port, the default Java port of{" "}
				<span style={specialWordStyle}>25565</span> will be used.
			</Typography>
		),
		response: {
			host: "hypixel.net",
			...
			version: {
				name: {
					raw: "Requires MC 1.8 / 1.21",
					clean: "Requires MC 1.8 / 1.21",
					html: '<span><span style="">Requires&nbsp;MC&nbsp;1.8&nbsp;/&nbsp;1.21</span></span>',
					json: '{"1":{"text":"Requires MC 1.8 / 1.21","styles":[]}}',
					segments: [
						{
							text: "Requires MC 1.8 / 1.21",
							styles: [],
						},
					],
				},
			...
			description: {
				raw: "§f                 §aHypixel Network §c[1.8/26.2]\n§f   §6§lSB 0.27 TORRHUS & SAFARI §7| §e§lSUMMER EVENT",
				clean: "                 Hypixel Network [1.8/26.2]\n   SB 0.27 TORRHUS & SAFARI | SUMMER EVENT",
				html: '<span><span style="color: #ffffff; ">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span><span style="color: #55ff55; ">Hypixel&nbsp;Network&nbsp;</span><span style="color: #ff5555; ">[1.8/26.2]<br /></span><span style="color: #ffffff; ">&nbsp;&nbsp;&nbsp;</span><span style="color: #ffaa00; font-weight: bold; ">SB&nbsp;0.27&nbsp;TORRHUS&nbsp;&amp;&nbsp;SAFARI&nbsp;</span><span style="color: #aaaaaa; ">|&nbsp;</span><span style="color: #ffff55; font-weight: bold; ">SUMMER&nbsp;EVENT</span></span>',
				json: '{"1":{"text":"                 ","styles":["color=#ffffff"]},"2":{"text":"Hypixel Network ","styles":["color=#55ff55"]},"3":{"text":"[1.8/26.2]\\n","styles":["color=#ff5555"]},"4":{"text":"   ","styles":["color=#ffffff"]},"5":{"text":"SB 0.27 TORRHUS & SAFARI ","styles":["color=#ffaa00","bold"]},"6":{"text":"| ","styles":["color=#aaaaaa"]},"7":{"text":"SUMMER EVENT","styles":["color=#ffff55","bold"]}}',
				segments: [
					{
						text: "                 ",
						styles: ["color=#ffffff"],
			...
			latency: 66,
			obtained_at: "2023-05-19T18:14:51.3294472-07:00",
			expires_at: "2023-05-19T18:19:51.3294472-07:00",
		},
	},
	{
		...
		description: (
			...
			>
				...
				replaced with the server's hostname or IP address. A valid IP
				address can be{" "}
				<span style={specialWordStyle}>{"play.hyperlandsmc.net"}</span>{" "}
				or{" "}
				<span style={specialWordStyle}>
					{"play.hyperlandsmc.net:19132"}
				</span>
				. If you do not provide a port, the default Bedrock port of{" "}
				<span style={specialWordStyle}>{"19132"}</span> will be used.
			</Typography>
		),
		response: {
	...
	},
	{
		...
		description: (
			...
			>
				...
				<span style={specialWordStyle}>base64</span>. If the server
				doesn't have an icon set or the server is offline, the default
				Minecraft server icon will be returned.{" "}
				<span style={specialWordStyle}>{"<ip>"}</span> should be
				replaced with the server's hostname or IP address. A valid IP
				address can be{" "}
				<span style={specialWordStyle}>{"hypixel.net"}</span> or{" "}
				<span style={specialWordStyle}>{"hypixel.net:25565"}</span>. If
				you do not provide a port, the default Java port of{" "}
				<span style={specialWordStyle}>{"25565"}</span> will be used.
			</Typography>
		),
		response: {
			host: "minehut.com",
			port: 25565,
			data: "data:image/png;base64,",
			obtained_at: "2023-05-25T01:25:02.718884931Z",
			expires_at: "2023-05-25T01:55:02.718885051Z",
		},
	},
	{
		title: "SRV Record",
		...
		description: (
			<Typography
				...
				paddingRight={2}
			>
				Returns the SRV redirect for the given Minecraft Java server
				host. If the host is not a SRV record, it will return the host
				as the IP and the port as 25565. A valid host would be{" "}
				<span style={specialWordStyle}>minehut.com</span>.
			</Typography>
		),
		response: {
			target: "java.cox.minehut.com.",
			port: 25565,
			obtained_at: "2023-05-25T01:25:48.388355033Z",
			expires_at: "2023-05-25T01:55:48.388355153Z",
		},
	},
];
